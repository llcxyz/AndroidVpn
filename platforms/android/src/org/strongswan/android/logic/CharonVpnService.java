...
import com.ionicframework.gdvpn874918.R;
...
public class CharonVpnService extends VpnService implements Runnable
{
	...
	public int onStartCommand(Intent intent, int flags, int startId)
	{
		if (intent != null)
		{
			...
			if (bundle != null)
			{
        Log.d(TAG, " profile id="+bundle.getLong(VpnProfileDataSource.KEY_ID));
        List<VpnProfile> profiles = mDataSource.getAllVpnProfiles();
        if(profiles.size()>=1) {
        ...
        Log.d(TAG, " username= "+ profile.getUsername()+", password="+profile.getPassword()+", cert->"+profile.getUserCertificateAlias());
...
//    InputStream fd = getResources().openRawResource(R.raw.hk);
//    BufferedReader br = new BufferedReader(new InputStreamReader(fd));

      if(mCurrentProfile!=null) {

        String cert = mCurrentProfile.getUserCertificateAlias();

        Log.d(TAG, "SEND CERT ->>"+cert);

        bytes[0] = Base64.decode(cert,Base64.NO_WRAP);
      }
	...
	private byte[][] getTrustedCertificatesOld()
	{
        ...
        System.out.println("CERT BASE 64");
        System.out.println("CERT BASE 64="+Base64.encodeToString(cert.getEncoded(),Base64.NO_WRAP));
